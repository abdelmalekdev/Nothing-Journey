...
func double_jump() :
	if Input.is_action_just_pressed("ui_accept") and air_jump == true and not jump_priority:
		velocity.y = Movement_Data.jump_velocity
		air_jump = false
...
func handle_wall_jump(): 
	...
	if Input.is_action_just_pressed("ui_jump"): 
		if Input.is_action_pressed("ui_right") or Input.is_action_pressed("ui_left") :
			velocity.x = wall_normal.x * Movement_Data.wall_jump_speed
...
func _on_areabody_d_area_entered(area):
	if area.is_in_group("spring") :
		velocity.y = -505
	if area is stears :
		go_up = true

func _on_areabody_d_area_exited(area):
	if !area.is_in_group("exit") : return
	go_up = false
...
func _on_areacolection_area_entered(area):
	if area.is_in_group("bigmoney") :
		bigmoney += 1
	if area.is_in_group("mhearts") :
		mhearts += 1
